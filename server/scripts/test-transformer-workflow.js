...
async function authRegisterAndLogin() {
  const ts = Date.now();
  const email = `transformer_test_${ts}@example.com`;
  const password = 'Test12345!';
  const username = `transformer_test_${ts}`;
...
async function createTransformerModel(token, inputShape = [50, 10], numClasses = 2) {
  const body = await jsonFetch(`${BASE_URL}/api/model/create`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${token}`
    },
    body: JSON.stringify({ inputShape, numClasses, modelType: 'transformer' })
  });
...
async function rocCurve(token, modelId, dataset = 'test') {
  const res = await fetch(`${BASE_URL}/api/training/roc-curve?modelId=${encodeURIComponent(modelId)}&dataset=${encodeURIComponent(dataset)}`, {
    headers: { Authorization: `Bearer ${token}` }
  });
  const body = await res.json();
  if (!res.ok) {
    throw new Error(`ROC curve failed: ${body?.error || body?.message || JSON.stringify(body)}`);
  }
  return body;
}

async function featureImportance(token, modelId, topK = 10) {
  const res = await fetch(`${BASE_URL}/api/training/feature-importance?modelId=${encodeURIComponent(modelId)}&topK=${encodeURIComponent(String(topK))}`, {
    headers: { Authorization: `Bearer ${token}` }
  });
  const body = await res.json();
  if (!res.ok) {
    throw new Error(`Feature importance failed: ${body?.error || body?.message || JSON.stringify(body)}`);
  }
  return body;
}

async function main() {
  console.log('=== TRANSFORMER automated workflow test ===');
  ...
  const classes = Math.max(2, Number(process.env.CLASSES || 2));

  // 1) Generate sample csv
  const csvPath = path.join(process.cwd(), 'datasets', 'sample_transformer_sequence.csv');
  execFileSync(
    process.execPath,
  ...
  const dsClasses = Number(s?.classes || 0);
  if (!inputShape.length || inputShape.length !== 2 || labelFormat !== 'one_hot' || dsClasses < 2 || dsClasses > 50) {
    throw new Error(
      ...
      `Got inputShape=${JSON.stringify(inputShape)} labelFormat=${labelFormat} classes=${dsClasses}.`
    );
  }

  // 4) Create model
  const created = await createTransformerModel(token, [timesteps, features], classes);
  ...
  try {
    const rocRes = await rocCurve(token, created.modelId, 'test');
    console.log('OK ROC:', { auc: rocRes?.auc, points: Array.isArray(rocRes?.points) ? rocRes.points.length : 0 });
  } catch (e) {
    console.warn('WARN ROC skipped:', e?.message || String(e));
  }

  // 9) Feature importance (optional)
  try {
    const fiRes = await featureImportance(token, created.modelId, 10);
    console.log('OK feature importance:', {
      method: fiRes?.method,
      totalFeatures: fiRes?.totalFeatures,
      topFeature: fiRes?.topFeature
    });
  } catch (e) {
    console.warn('WARN feature importance skipped:', e?.message || String(e));
  }
